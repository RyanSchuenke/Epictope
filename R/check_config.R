#' @title check_config
#'
#' @description Checks if a configuration file exists and loads the configuration parameters. If the file does not exist, it sets default values for the configuration parameters.
#'
...
check_config <- function() {
  ...
  if (length(files) > 0) {
    message("Found custom configuration file. Using custom values.")
    source(files[1])
  } else {
    message("Using default configuration values.")
